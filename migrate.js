...
async function migrate() {
  ...
  try {
    ...
    // Seed root admin (idempotent — runs after all migrations so schema is ready)
    const seedPath = path.join(__dirname, 'scripts', 'seed-root-admin.js');
    try {
      require(seedPath);
    } catch (e) {
      // seed-root-admin logs its own errors; non-fatal if it fails
      console.log('[migrate] seed-root-admin: skipped or errored (', e.message, ')');
    }
  } finally {
    client.release();
    await pool.end();
...
async function runFolderMigrations(client) {
  ...
  for (const file of files) {
    const name = file.replace(/\//g, '_').replace(/\\/g, '_'); // cross-platform safe name
    ...
    try {
      ...
      if (file.endsWith('.sql')) {
        // Raw SQL file — read and execute as-is
        const sql = fs.readFileSync(path.join(migrationsDir, file), 'utf8');
        await client.query(sql);
      } else {
        // JS module with .up(client) function
